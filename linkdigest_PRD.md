# Link Digest 북마크 요약 서비스 요구사항 명세서 (PRD)

## 문서 정보

- **문서명**: Link Digest 서비스 요구사항 명세서
- **버전**: 1.2
- **작성일**: 2026-02-14
- **최종 업데이트**: 2026-02-14
- **작성자**: Link Digest 개발팀
...
### 3.2 공개 북마크 (비로그인) 관련
- **US-004-1**: 비로그인 사용자는 첫 화면(`/main`)에서 공개된 북마크 목록을 로그인 후 목록과 동일한 UI로 조회할 수 있어야 한다.
- **US-004-2**: 비로그인 사용자는 공개 북마크를 클릭하여 상세를 볼 수 있으며, 상세에서는 요약만 표시되고 컨텐츠 버튼은 제공하지 않는다.
- **US-004-3**: API는 공개용(인증 불필요)과 인증용으로 구분되어 정의·사용되어야 한다.
...
#### FR-004-1: 공개 북마크 목록 조회
- **우선순위**: 필수 (Must Have)
- **설명**: 비로그인 사용자는 `is_public=True`인 북마크 목록만 인증 없이 조회할 수 있어야 한다.
- **입력**: page, per_page (쿼리 파라미터)
- **처리**: GET `/api/public/bookmarks/`, 인증 의존성 없음, CRUD `get_multi_public`, `count_public` 사용
- **출력**: BookmarkListResponse (items, total, page, per_page, total_pages)

#### FR-004-2: 공개 북마크 단건 조회
- **우선순위**: 필수 (Must Have)
- **설명**: 비로그인 사용자는 `is_public=True`인 북마크 단건을 인증 없이 조회할 수 있어야 한다.
- **입력**: bookmark_id (UUID, 경로 파라미터)
- **처리**: GET `/api/public/bookmarks/{bookmark_id}`, CRUD `get_public_by_id` 사용
- **출력**: BookmarkResponse. 해당 북마크가 공개가 아니거나 없으면 404.

#### FR-004-3: 첫 화면 및 라우팅
- **우선순위**: 필수 (Must Have)
- **설명**: 서비스 첫 화면 URL은 `/main`이며, 공개 북마크 목록을 표시한다. 알 수 없는 경로는 `/main`으로 리다이렉트. 401 발생 시 `/main`으로 리다이렉트.
- **프론트**: 공개용 API(`api.publicBookmarks`)와 인증용 API(`api.bookmarks` 등)를 구분하여 정의·사용.

#### FR-004-4: 공개 북마크 상세 보기 제한
- **우선순위**: 필수 (Must Have)
- **설명**: 비로그인 사용자가 공개 북마크 상세를 볼 때는 요약만 표시하고, 원문(컨텐츠) 전환 버튼은 제공하지 않는다. 조회수 증가 및 요약 폴링은 실행하지 않는다.
...
#### FR-011: 북마크 수정
- **우선순위**: 필수 (Must Have)
- **설명**: 사용자는 북마크를 수정할 수 있어야 한다. 다른 사용자가 공유한 글은 수정할 수 없다.
...
- **처리**:
  ...
  - DB 업데이트
- **출력**: 수정된 북마크 정보
- **에러 처리**: 권한 없음 시 403 에러, 응답 `detail: "권한 없음"`

#### FR-012: 북마크 삭제
- **우선순위**: 필수 (Must Have)
- **설명**: 사용자는 북마크를 삭제할 수 있어야 한다. 다른 사용자가 공유한 글은 삭제할 수 없다.
- **입력**: 북마크 ID
- **처리**:
  - 북마크 ID로 조회 후 소유자 확인
  - 삭제 권한 검증
...
- **출력**: 성공 메시지
- **에러 처리**: 북마크 없음 404, 권한 없음 시 403 에러, 응답 `detail: "권한 없음"`
...
### 2026-02-12: 공유 글 권한 및 글 보기 UI 반영
- **기능**: FR-011·FR-012에 타인 공유 글 수정/삭제 불가, 403 시 `detail: "권한 없음"` 명시. FR-012 삭제 시 ID 조회 후 소유자 검증(404/403 구분).
- **비기능**: NFR-011에 공유 글 권한 UI(수정/삭제 아이콘 등록자만, 공개 아이콘 색 구분), 글 보기 화면(뒤로가기·제목 같은 줄, 제목 클릭 목록 복귀, 반응형·모바일 제목 표시) 요구사항 추가.
